...
var onrequest = function (req, res) {
  ...
  if (!req.upgrade) {
    // 非upgrade请求选择：中断或提供普通网页 res.writeHead(200, { 'content-type': 'text/plain' });
...
var wrap = function (data) {
  ...
  len = 2 + Buffer.byteLength(data),
    buff = new Buffer(len);
  buff[0] = fa; buff.write(data, 1);
  buff[len - 1] = fe; return buff;
}
...
var bind_sock_event = function (sock) {
  sock.on('data', function (buffer) {
    var data = unwrap(buffer); console.log('socket receive data : ', buffer, data, '\n>>> ' + data);
...
var get_part = function (key) {
  var empty = '',spaces = key.replace(/\S/g, empty).length, part = key.replace(/\D/g, empty);
...
var handshake = function (req, sock, head) {
  ...
  output.push(c); sock.write(output.join(br), 'binary');
}